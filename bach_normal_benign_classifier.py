"""
BACH Normal vs Benign Binary Classifier
Specialized binary classification for non-malignant tissue subtypes
...
class BACHNormalBenignClassifier:
    def __init__(self, cache_file='/workspace/embeddings_cache_4_CLUSTERS_FIXED_TSNE.pkl'):
        ...
        self.class_names = ['normal', 'benign']
        
    def load_bach_normal_benign_data(self):
        """Load only normal and benign samples from BACH dataset"""
        print("Loading BACH normal vs benign data from cache...")
        ...
        # Filter for only normal and benign samples
        normal_benign_indices = []
        for idx in bach_indices:
            label = combined_data['labels'][idx]
            if label in ['normal', 'benign']:
                normal_benign_indices.append(idx)
        
        if not normal_benign_indices:
            raise ValueError("No normal/benign samples found in BACH data")
        
        # Get filtered features and labels
        features = np.array([combined_data['features'][i] for i in normal_benign_indices])
        labels = [combined_data['labels'][i] for i in normal_benign_indices]
        filenames = [combined_data['filenames'][i] for i in normal_benign_indices]
        
        print(f"Loaded BACH normal vs benign data: {len(features)} samples")
    ...
    def train_classifiers(self):
        """Train all three classifiers for normal vs benign"""
        features, labels, filenames = self.load_bach_normal_benign_data()
    ...
    def _train_logistic_regression(self, X_train, X_val, X_test, y_train, y_val, y_test):
        ...
        print("\n🔥 Training Normal vs Benign Logistic Regression...")
    ...
    def _train_svm(self, X_train, X_val, X_test, y_train, y_val, y_test):
        """Train SVM RBF"""
        print("\n🔥 Training Normal vs Benign SVM RBF...")
    ...
    def _train_xgboost(self, X_train, X_val, X_test, y_train, y_val, y_test):
        """Train XGBoost"""
        print("\n🔥 Training Normal vs Benign XGBoost...")
    ...
    def save_model(self, model_path='/workspace/bach_normal_benign_model.pkl'):
        """Save trained models"""
        model_data = {
            ...
            'task': 'normal_vs_benign'
        }
        
        with open(model_path, 'wb') as f:
            pickle.dump(model_data, f)
        
        print(f"Normal vs Benign model saved to: {model_path}")
        return model_path
    
    def load_model(self, model_path='/workspace/bach_normal_benign_model.pkl'):
        ...
        try:
            ...
            print(f"Normal vs Benign model loaded from: {model_path}")
            ...
            print(f"Normal vs Benign model file not found: {model_path}")
            return False
        except Exception as e:
            print(f"Error loading Normal vs Benign model: {e}")
            return False

def train_normal_benign_classifier():
    """Train the normal vs benign classifier"""
    classifier = BACHNormalBenignClassifier()
    ...
    if success:
        ...
        print(f"\n✅ BACH Normal vs Benign Classifier Training Complete!")
...
if __name__ == "__main__":
    classifier = train_normal_benign_classifier()